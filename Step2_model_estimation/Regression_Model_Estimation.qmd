...
getIndex_9099 = function(df){
  df = df %>%
    # 计算APE = （Actual - Predict）/ Actual
    mutate(
      APE_Grump = abs(UN -Grump ) / UN,
      APE_GPW = abs(UN -GPW ) / UN,
      APE_GHS = abs(UN -GHS ) / UN
    ) %>%
    # 计算SE = （Actual - Predict）^2
    mutate(
      SE_Grump = (UN -Grump )**2,
      SE_GPW = (UN -GPW )**2,
      SE_GHS = (UN -GHS )**2
  ) %>%
    # 计算SLE = ( ln(1+x)-ln(1+y))^2
    mutate(
...
```{R }
# train data：unclustered 
train.data_whole  <- train_df[,c(4:7)] # year = 1990
...
```{R}
setwd("F:/Code/R_file/2022_12/Country_cluster/")
...
get_t_times_accuracy = function(t) {
  ...
  # 2000
  getOutput_2000 = function (annum) {
    filepath = paste0("F:/Code/R_file/2022_12/Country_cluster/",
                      "pop_3set_00to20.xlsx")
...
```{R}
txt_path = paste0("E:/Out_Fig/Fig_table/Fig_A4_repeat_training_best_times")
setwd(txt_path)
